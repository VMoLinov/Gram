...
class GroupChatFragment :
    ...
    private val loadAttach = registerForActivityResult(CropImageContract()) { result ->
        if (result.isSuccessful) {
            uploadFile(result.uriContent, group.id, TYPE_MESSAGE_IMAGE)
            isSmoothScroll = true
        } else showToast(result.error.toString())
    }
    private val getContent = registerForActivityResult(ActivityResultContracts.GetContent()) {
        it?.let { uploadFile(it, group.id, TYPE_MESSAGE_FILE) }
    }

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)
        group = arguments?.getParcelable(ARGS_KEY) ?: CommonModel()
        toolbar = MAIN_ACTIVITY.toolbar.findViewById(R.id.toolbarInfo)
        refUser = REFERENCE_DB.root.child(NODE_USERS).child(group.id)
        refMessages = REFERENCE_DB.child(NODE_GROUPS).child(group.id).child(NODE_MESSAGES)
    }
    ...
    private fun initToolbarInfo() {
        toolbar.apply {
            findViewById<TextView>(R.id.toolbarFullName).text =
                receivingUser.fullName.ifEmpty { group.fullName }
    ...
    private fun setSent() = with(binding) {
        btnSent.setOnClickListener {
            ...
            else sendMessageToGroup(enterMessage, group.id) {
            ...
            btnVoice.setOnTouchListener { _, motionEvent ->
                if (checkPermission(RECORD_AUDIO)) {
                    if (motionEvent.action == MotionEvent.ACTION_DOWN) {
                        ...
                        voiceRecorder.startRecording(group.id)
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            R.id.clear -> clearChat(group.id) {
                showToast(getString(R.string.single_chat_cleared))
                replaceFragment(MainListFragment())
            }
            R.id.delete -> deleteChat(group.id) {
